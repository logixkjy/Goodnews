...
@interface ViewController () <WKUIDelegate, WKNavigationDelegate, MFMessageComposeViewControllerDelegate, MFMailComposeViewControllerDelegate>
...
@property (weak, nonatomic) IBOutlet UIButton *m_homeButton;
@property (weak, nonatomic) IBOutlet UIButton *m_prevButton;
@property (weak, nonatomic) IBOutlet UIButton *m_nextButton;
@property (weak, nonatomic) IBOutlet UIButton *m_reflashButton;
@property (weak, nonatomic) IBOutlet UIButton *m_moreButton;
@property (weak, nonatomic) IBOutlet UIButton *m_moveTopButton;
...
- (void)viewDidLoad
{
    ...
    [self.m_homeButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id  _Nonnull sender) {
        GetDataCenter.MAIN_URLstring = GetDataCenter.str_MAIN_URL;
        [self loadMainUrl];
    }];
    ...
    [self.m_reflashButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id  _Nonnull sender) {
    ...
    [self.m_moveTopButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id  _Nonnull sender) {
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest
                                    requestWithURL:[[NSURL alloc] initWithString:MAIN_URL]
                                    cachePolicy:NSURLRequestReloadIgnoringCacheData
                                    timeoutInterval:60.0];
    NSError *error  = nil;
    NSData *dataBuffer = [GPCommonUtil sendSynchronousRequest:request returningResponse:nil error: &error];
    if (error) {
        
    } else {
        TBXML *tbxml = [[TBXML alloc] initWithXMLData:dataBuffer error:&error];
        if (error) {
            NSLog(@"link [%@]",error.description);
            return;
        }
        TBXMLElement *xmlRss = tbxml.rootXMLElement;
        if (xmlRss) {
            NSLog(@"%@",[TBXML textForElement:[TBXML childElementNamed:@"link" parentElement:xmlRss]]);
            GetDataCenter.str_MAIN_URL = [TBXML textForElement:[TBXML childElementNamed:@"link" parentElement:xmlRss]];
        }
    }
...
- (void)setLayout
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        ...
        self.m_homeButton.frame = CGRectMake(gap, 0, RESIZE(50), RESIZE(50));
        self.m_prevButton.frame = CGRectMake(CGRectGetMaxX(self.m_homeButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
        self.m_nextButton.frame = CGRectMake(CGRectGetMaxX(self.m_prevButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
        self.m_reflashButton.frame = CGRectMake(CGRectGetMaxX(self.m_nextButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
        self.m_moreButton.frame = CGRectMake(CGRectGetMaxX(self.m_reflashButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
        self.m_moveTopButton.frame = CGRectMake(MAIN_SIZE().width - RESIZE(50) - gap, 0, RESIZE(50), RESIZE(50));
        ...
        self.m_reflashButton.frame = CGRectMake(CGRectGetMaxX(self.m_nextButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
        self.m_moreButton.frame = CGRectMake(CGRectGetMaxX(self.m_reflashButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
        self.m_moveTopButton.frame = CGRectMake(CGRectGetMaxX(self.m_moreButton.frame) + gap, 0, RESIZE(50), RESIZE(50));
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([GetDataCenter.MAIN_URLstring length] == 0) {
            GetDataCenter.MAIN_URLstring = GetDataCenter.str_MAIN_URL;
            NSURLRequest *req = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:GetDataCenter.MAIN_URLstring]];
...
- (BOOL)scriptControl:(NSString*)strUrl {
    ...
    if (range.location != NSNotFound) {
        [GetDataCenter setPUSH_URL:[strUrl stringByReplacingOccurrencesOfString:MAIN_KEY withString:@"https://"]];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if ([strUrl hasPrefix:PCVIEW]) {
        ...
        if (!GetDataCenter.isShowModalView) {
            ...
            self.modalPCView = modalPCView;

            GetDataCenter.isShowModalView = YES;

            dispatch_async(dispatch_get_main_queue(), ^{
                [self presentViewController:self.modalPCView animated:YES completion:nil];
            });
            decisionHandler(WKNavigationActionPolicyCancel);
            return;
        }
    ...
    if (([strUrl length] < 6)||[strUrl isEqualToString:@"(null)"]) {
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
    }
